<template>
  <div class="inline-block h-6 w-6 ">
    <button class="h-6 w-6 inline-block cursor-pointer" @click="handleShare">
      <Icon name="material-symbols:share" class="text-2xl" />
    </button>

		<!-- Drawer Content -->
		<DrawerCustom v-model:open="showDrawer" title="All Share Menu" description="Kelola Share Menu">
				<AppContainer>
					<div class="flex flex-col gap-3.5 my-4 mx-auto justify-center">
						...
						<div class="flex flex-wrap justify-center gap-y-10 px-4 my-2 -mx-4">
							<template v-for="social in socials" :key="social.id">
								<div
									class="flex flex-col justify-start items-center cursor-pointer w-1/4"
